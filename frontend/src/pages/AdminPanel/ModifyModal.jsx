  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSave)} className="flex flex-col  gap-12 items-center">
          ...
          <input
            ...
            className="border-2 rounded-2xl h-3/16 w-6/10 p-8 text-[2rem]"
          />
          {errors.dates && <p className="text-red-600">{errors.dates.message}</p>}
          <input
            {...register("category_id", { required: "Category ID is required" })}
            placeholder="Category ID"
            className="border-2 rounded-2xl h-3/16 w-6/10 p-8 text-[2rem]"
          />
          <button type="submit" className="bg-[var(--lighter-background-color)] rounded-2xl border-2 h-3/16/16 w-3/16 text-[1.75rem] justify-center max-xl:text-[1.25rem] max-xl:h-3/16">
            Save
          </button>
        </form>
      </div>
