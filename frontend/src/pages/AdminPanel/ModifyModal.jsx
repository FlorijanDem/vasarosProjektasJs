...
const ModifyExcursionModal = ({ isOpen, onClose, excursion, onSave }) => {
  ...
  return (
    ...
    >
      <div
        className="rounded bg-white p-8 relative w-1/3 shadow-2xl text-center flex gap-8 flex-col max-md:w-9/10"
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col  gap-12 items-center">
          <input
          ...
          />
          {errors.title && <p className="text-red-600">{errors.photo_url.message}</p>}
          <input
            {...register("duration", { required: "Duration is required" })}
            placeholder="Duration"
            className="border-2 rounded-2xl h-3/16 w-6/10 p-8 text-[2rem]"
          />
          {errors.title && <p className="text-red-600">{errors.duration.message}</p>}
          <input
            {...register("dates", { required: "Date is required" })}
            placeholder="Date"
            className="border-2 rounded-2xl h-3/16 w-6/10 p-8 text-[2rem]"
          />
          {errors.title && <p className="text-red-600">{errors.dates.message}</p>}
          <input
            {...register("category_id", { required: "Category ID is required" })}
            placeholder="Category ID"
            className="border-2 rounded-2xl h-3/16 w-6/10 p-8 text-[2rem]"
          />
          <button type="submit" className="bg-white rounded-2xl border-2 h-3/16/16 w-3/16 text-[1.75rem] justify-center max-xl:text-[1.25rem] max-xl:h-3/16">
            Save
          </button>
        </form>
      </div>
